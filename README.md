...
Требуется `tkinter` (Tcl/Tk). Он не устанавливается через `pip`.

Если вы запускаете из Linux/WSL и получаете ошибку `No module named 'tkinter'`, установите пакет для вашей ОС (например, `python3-tk` или `tk`) или используйте консольный режим.
